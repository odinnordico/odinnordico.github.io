...
var PdfCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		targetLang := utils.GetLang(viper.GetString("lang"))
		...
		return GenerateMultiLanguagePdf(dataDir, outputDir, targetLang, theme)
	},
}
...
// and available language directories. If targetLang is empty or "en", it auto-detects all available languages.
func detectLanguages(dataDir, targetLang string) []string {
	...
	filepath.WalkDir(langDir, func(path string, d os.DirEntry, err error) error {
		...
		if d.IsDir() && d.Name() != "en" && d.Name() != langDirName {
...
// If targetLang is empty or "en", it auto-detects all available languages.
